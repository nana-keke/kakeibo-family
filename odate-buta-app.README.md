...
- 左右傾斜ボタンのラベル（`lrQuickLabel()`）は「左0.2」「右0.2」のようなテキストではなく、矢印＋数値（左側のボタンは`↙0.2`、右側のボタンは`↘0.2`）で表示。**矢印の向きはボタンの見た目上の並び位置に合わせたものであり、物理的な下り坂方向とは対応していない**（ユーザー確認済み。内部の記録値・符号・集計ロジックは変更なし）
