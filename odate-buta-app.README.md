...
- **「デモ用に値を入力」**（`fillDemoData()`）: 皿15・役物入賞玉数45・当選経路各種・ラウンド・はたき系などを一括で`addEvent()`し、`session.isDemo = true`を立てる。値は`countValidWarnings()`の警告が出ない範囲（総V入賞回数とラウンド総数が一致する等）に調整済み。実行後は通常のボタンで実際にタップ操作を追加でき、そのまま実戦終了してカレンダー・集計タブを見せられる
