...
- **皿単価あたりの期待出玉（`r.expectedPayoutPerPlate` = A）**: `皿単価 × 役物当選率 × 1当選あたり平均出玉（AVG_PAYOUT≒566.67玉）`。単位は「玉／皿」で、現在の皿単価・当選率を実際の払出しベースに換算した値。`m.expectedPayoutPerPlate` として集計タブの主要指標（皿単価の直下）に表示。終了済みセッションでも算出される（時間の概念を含まないため）
